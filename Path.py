...
class Node:
    ...
    def successors(self, numberToFill: int, numberFree=0) -> List[Node]:
        ...
        for i in range(4):
            ...
            newDirectionChanges = self.directionsChanges + \
                (1 if self.previousDirection != i else 0)
            if exists(newOrigin, self.matrix) and self.matrix[newOrigin] == numberFree:
                ...
                suc.append(Node(newMatrix, newOrigin, self.destiny,
                           self.length+1, i, newDirectionChanges))
            if newOrigin == self.destiny:
                suc.append(Node(self.matrix, newOrigin, self.destiny,
                           self.length, i, newDirectionChanges))
        return suc
        ...
        for destiny in [indice for indice in indicesOfNumberInMatrix(matrix, numberD) if indice != origin]:
            if corner:
                if isCornerOfPath(matrix, destiny):
                    continue
            l, m = connectOrigDest(
                matrix, origin, destiny, caminoNumber if caminoNumber != None else numberD, numberFree, debug=debug)
            if type(l) != type(None):
...
def connectOrigDest(matrix: np.ndarray, origin: Tuple[int, int], destiny: Tuple[int, int], caminoNumber, numberFree=0, debug=False) -> Tuple[int, np.ndarray] | Tuple[None, None]:
    ...
    queue.put(distance(origin, destiny), Node(matrix, origin,
              destiny, 0, None if cape == -1 else cape, 0))
        ...
        for sucesor in node.successors(caminoNumber, numberFree=numberFree):
            ...
            if not sucesor.origin in visited:
                ...
                if d == 0:
                    ...
                    queue.put(d+sucesor.length +
                              sucesor.directionsChanges*2, sucesor)
    return None, None
...
def connected(pd: np.ndarray, strand: Strand):
    capes = [ind for ind in indicesOfNumberInMatrix(
        pd, strand) if isCapeOfCross(pd, ind) != -1]
    if len(capes) != 2:
        return False, None
