...
class IndividualKnot:
    ...
    def mutate(self, rateType: float = 0.05, rateGene: float = 0.1, rateGenes: float = 0.05):
        ...
        self.genome.mutate(rateType=rateType,
                           rateGene=rateGene, rateGenes=rateGenes)


def crossover2(motherGenome: Genome, fatherGenome: Genome):
    motherPoint = randrange(len(motherGenome.genes)) if len(
        motherGenome.genes) != 0 else 0
    fatherPoint = randrange(len(fatherGenome.genes)) if len(
        fatherGenome.genes) != 0 else 0
    daughterGenome = Genome(deepcopy(
        motherGenome.genes[:motherPoint]) + deepcopy(fatherGenome.genes[fatherPoint:]))
    sonGenome = Genome(deepcopy(
        fatherGenome.genes[:fatherPoint]) + deepcopy(motherGenome.genes[motherPoint:]))
...
def crossover(motherGenome: Genome, fatherGenome: Genome):
    if len(motherGenome.genes) != 0 and len(fatherGenome.genes) != 0:
        point1Aux = randrange(
            min(len(motherGenome.genes), len(fatherGenome.genes)))
        point2Aux = randrange(
            min(len(motherGenome.genes), len(fatherGenome.genes)))
    else:
        point1Aux = 0
...
class PopulationKnot:
    ...
    def selection(self, times: bool = False, debug=0):
        ...
        for i in range(len(self.population)):
            ...
            if debug > 0:
                print("  percentage selection calculate Difference: {:3.2f}   ".format(
                    percent), end="\r")
        ...
        if debug > 0:
            ...
            print("differences len: ", len(self.differences),
                  "differences calls: ", self.differences._calls)
        ...
        newPopulation: List[IndividualKnot] = list(np.random.choice(
            self.population, self._numberOfIndividuals, p=listDifference, replace=False))
    ...
    def crossover(self, debug=0):
        ...
        while len(self.population) > 1:
            # start = time()
            if debug > 0:
                print("  crossover: remaining individuals: {}   ".format(
                    len(self.population)), end="\r")
    ...
    def mutate(self, debug=0):
        for i in range(len(self.population)):
            percent = ((i+1)/len(self.population))*100
            if debug > 0:
                print("  percentage mutate: {:3.2f}   ".format(
                    percent), end="\r")
            individual = self.population[i]
            while random() < MutationRate:
                individual.mutate(rateType=self.mutationRateType,
                                  rateGene=self.mutationRateGene, rateGenes=self.mutationRateGenes)
        if debug > 0:
            print(
                "                                                                 ", end="\r")
    ...
    def updateMutationRates(self):
        self.mutationRateType = remap(
            self._generation, 0, self._numberOfGenerations, self._maxMutationRateType, self._minMutationRateType)
        self.mutationRateGene = remap(
            self._generation, 0, self._numberOfGenerations, self._maxMutationRateGene, self._minMutationRateGene)
        self.mutationRateGenes = remap(
            self._generation, 0, self._numberOfGenerations, self._maxMutationRateGenes, self._minMutationRateGenes)
...
def areSameKnotGA(knot1: CustomKnot,
                  knot2: CustomKnot,
                  ...
                  minMutationRateGenes=0.003) -> Tuple[bool, IndividualKnot | None]:
    if knot1 == knot2:
...
def plotDifferences(differences):
    fig, ax = plt.subplots()
    ax.plot(range(1, len(differences[0])+1),
            differences[0], color='tab:blue', label="Max")
    ax.plot(range(1, len(differences[1])+1),
            differences[1], color='tab:green', label="Mean")
    ax.plot(range(1, len(differences[2])+1),
            differences[2], color='tab:red', label="Min")
