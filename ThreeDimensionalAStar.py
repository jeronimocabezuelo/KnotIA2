...
class ThreeDimensionalPoint:
    ...
    def ninePointsInTwoAxis(self,axis1: Axis|AxisDirection, axis2: Axis|AxisDirection):
        aux: list[ThreeDimensionalPoint] = [self] + self.eightPointsInTwoAxis(axis1,axis2)
        return aux
...
def directionFromPoint(fromPoint: ThreeDimensionalPoint, toPoint: ThreeDimensionalPoint) -> AxisDirection:
    ...
    if fromPoint.x == toPoint.x and fromPoint.y == toPoint.y:
        if fromPoint.z < toPoint.z: return AxisDirection.ZU
        else: return AxisDirection.ZD
    if fromPoint.x == toPoint.x and fromPoint.z == toPoint.z:
        if fromPoint.y < toPoint.y: return AxisDirection.YU
        else: return AxisDirection.YD
    if fromPoint.y == toPoint.y and fromPoint.z == toPoint.z:
        if fromPoint.x < toPoint.x: return AxisDirection.XU
        else: return AxisDirection.XD
...
class FlooredMatrix:
    ...
    def checkPoint(self,point: ThreeDimensionalPoint):
        ...
        if t[1]<0: return AxisDirection.XD
        if t[2]<0: return AxisDirection.YD
        if t[0]<0: return AxisDirection.ZD
        if t[1]>=self.baseMatrix.shape[1]: return AxisDirection.XU
        if t[2]>=self.baseMatrix.shape[2]: return AxisDirection.YU
        if t[0]>=self.baseMatrix.shape[0]: return AxisDirection.ZU
        return None
    ...
    def addUpFloor(self):
        ...
        self.baseMatrix = np.insert(self.baseMatrix,self.baseMatrix.shape[0],zerosFloor,0)
...
class ThreeDimensionalTransform:
    ...
    def allPosibilitesForReduceLengthIn(point:ThreeDimensionalPoint):
        posibilites = [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.allInLine,False)]
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.allInAxis,None,d) for d in list(AxisDirection)]
        return posibilites
    ...
    def allPosibilitesIn(point):
        ...
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.inLine,True,d) for d in list(AxisDirection)]
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.doubleAllInLine,True,d) for d in list(AxisDirection)]
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.allInLine,True,d) for d in list(AxisDirection)]
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.switchSide,v,d) for d in list(AxisDirection) for v in [False, True]]
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.inCornerForSide,v,d) for d in list(AxisDirection) for v in [False, True]]
        posibilites += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.inLine,False),
                        ThreeDimensionalTransform(point,ThreeDimensionalTransformType.allInLine,False),
...
class FlooredKnot:
    ...
    def auxiliarPointsForThreeInLine(self, point: ThreeDimensionalPoint, direction: AxisDirection) -> tuple[list[ThreeDimensionalPoint],list[ThreeDimensionalPoint],list[ThreeDimensionalPoint]]:
        ...
        points = [point.move(firstAxis,firstDirection), point.move(firstAxis,firstDirection*2)]
        ...
        toCompletePoints = [toCompletePointCenter, toCompletePointCenterUp, toCompletePointCenterDw]
    ...
    def auxiliarPointsForUndoThreeInLine(self,point: ThreeDimensionalPoint):
        ...
        availableDirections = sum([[axis.axisDirection(d) for d in [-1,1]]for axis in availableAxis],[])
    ...
    def auxiliarPointsForDoubleAllInLine(self, point, direction: AxisDirection):
        ...
        pointsToComplete = list(set(firstPointsToComplete).union(set(secondsPointsToComplete)).difference(set(secondsPointsToDelete)))
        pointsToDelete = list(set(firstPointsToDelete).union(set(secondsPointsToDelete)).difference((set(secondsPointsToComplete))))
    ...
    def auxiliarPointsForUndoAllInLineTransform(self, point: ThreeDimensionalPoint):
        ...
        for direction in availableDirections:
            ...
            externalsPoints = [allPoints[0].move(direction),allPoints[-1].move(direction)]
            if self.isOne(externalsPoints):
                pointsNeedFree = [desiredPoint.move(direction) for desiredPoint in desiredPoints]
                otherAxis = Axis.thatAreNot([axis,direction.axis])
                pointsNeedFree = sum([p.twoPointInAxis(otherAxis) for p in pointsNeedFree], pointsNeedFree)
    ...
    def auxiliarPointsForAllInLineDoubleDirectionTransform(self, point: ThreeDimensionalPoint, variety: bool, previousDirection = None, withDirection = False):
        ...
        direction1:AxisDirection = None; direction2: AxisDirection = None, None
        ...
        external1, external2 = [external1,external2][variety], [external1,external2][not variety]
        direction1, direction2 = [direction1,direction2][variety] , [direction1,direction2][not variety]
        ...
        pointsNeedFree: list[ThreeDimensionalPoint] = external2.move(direction1).ninePointsInTwoAxis(axis,Axis.thatAreNot([axis,direction1.axis]))
        pointsNeedFree += external2.move(direction1).move(direction1).ninePointsInTwoAxis(axis,Axis.thatAreNot([axis,direction1.axis]))
    
        freePoints1 = allPoints[:-1] if variety else allPoints[1:]
        freePoints1 = move(freePoints1, direction1)
        freePoints1 += sum([p.twoPointInAxis(Axis.thatAreNot([axis,direction1.axis])) for p in freePoints1],[])
    ...
    def auxiliarPointsForAllInLineDoubleDirectionLoopTransform(self, point: ThreeDimensionalPoint, variety: bool):
        auxiliarPoints = self.auxiliarPointsForAllInLineDoubleDirectionTransform(point,variety, withDirection=True)
        ...
        while True:
            ...
            point = point.move(direction)
            auxiliarPoints = copy.auxiliarPointsForAllInLineDoubleDirectionTransform(point,variety, withDirection=True)
            if type(auxiliarPoints) == type(None):
                return list(pointsNeedFree), list(pointsToComplete), list(pointsToDelete)
    ...
    def directionsForThreeInCorner(self,point: ThreeDimensionalPoint):
        if self.isZero(point):
            return None
        axes = list(Axis)
        axesTuples = [(axes[i],axes[j]) for i in range(len(axes))for j in range(len(axes))if i<j]
        axesDirectionsTuples = [ (axis1.axisDirection(d1),  axis2.axisDirection(d2)) for axis1, axis2 in axesTuples for d1 in [-1,1] for d2 in [-1,1]]
    ...
    def auxiliarPointsForThreeInCorner(self, point: ThreeDimensionalPoint):
        ...
        pointsToComplete = [desiredPoint]
        pointsToDelete = [point]
        pointsNeedFree = self.fourPoints(desiredPoint,aD1,aD2) + self.fourPoints(upPoint,aD1,aD2) + self.fourPoints(dwPoint,aD1,aD2)
    ...
    def auxiliarPointsForSwitchSideOpposite(self, point: ThreeDimensionalPoint, aD1: AxisDirection, aD2: AxisDirection):
        ...
        moreDownFreePoints = sum([p.twoPointInAxis(aD1) for p in downFreePoints],[]) + sum([p.twoPointInAxis(aD1,2) for p in downFreePoints],[]) + [p.move(aD1,3) for p in downFreePoints]
        downFreePoints += moreDownFreePoints

        moreDownFreePoints = [p.move(aD2.opposite) for p in downFreePoints] + [p.move(aD2.opposite,2) for p in downFreePoints]
        downFreePoints += moreDownFreePoints
    ...
    def auxiliarPointsForSwitchSideTransversal(self,point:ThreeDimensionalPoint, direction: AxisDirection, aD1: AxisDirection, aD2: AxisDirection):
        ...
        basicPoints = sum([p.ninePointsInTwoAxis(direction,aD1) for p in basicPoints], [])
    ...
    def auxiliarPointsForSlideCorner(self,point: ThreeDimensionalPoint):
        ...
        if allInLine1[-1] == point: allInLine1.reverse()
        if allInLine2[-1] == point: allInLine2.reverse()
        ...
        cornerPointFree = point.move(aD1.opposite).move(aD2.opposite).move(outsideDirection)

        pointsNeedFree = freePoints1 + freePoints2 + [cornerPointFree, cornerPointFree.move(outsideDirection)]
    ...
    def auxiliarPointsForInCornerForSide(self, point: ThreeDimensionalPoint, variety: bool, direction: AxisDirection):
        ...
        auxFreePoints = point.move(direction.opposite,2).ninePointsInTwoAxis(direction, aD2)
        ...
        freePoints2 += [p.move(aD1.opposite) for p in freePoints2] + [p.move(aD1.opposite,2) for p in freePoints2]
    ...
    def auxiliarPointsForAllInAxis(self, point: ThreeDimensionalPoint, direction: AxisDirection):
        ...
        if self.isOne(points[0].move(direction)):
            extreme1FreePoints = points[1].move(direction).sixPointsInAxisAndDirection(otherDirection1, direction1.opposite)
            ...
            extreme1FreePoints = points[0].move(direction).sixPointsInAxisAndDirection(otherDirection1,direction1)
            extreme1FreePoints += points[1].move(direction).sixPointsInAxisAndDirection(otherDirection1,direction1.opposite)
            extreme1FreePoints += [p.move(direction) for p in extreme1FreePoints]
        ...
        if self.isOne(points[-1].move(direction)):
            extreme2FreePoints = points[-2].move(direction).sixPointsInAxisAndDirection(otherDirection2, direction2.opposite)
            ...
            extreme2FreePoints = points[-1].move(direction).sixPointsInAxisAndDirection(otherDirection2,direction2)
            extreme2FreePoints += points[-2].move(direction).sixPointsInAxisAndDirection(otherDirection2,direction2.opposite)
            extreme2FreePoints += [p.move(direction) for p in extreme2FreePoints]
        ...
        normalFreePoints: list[ThreeDimensionalPoint] = sum([p.move(direction).ninePointsInTwoAxis(otherAxes[0], otherAxes[1]) for p in normalPoints], [])
        ...
        pointsToComplete = extreme1ToComplete + [p.move(direction) for p in points[1:-1]] + extreme2ToComplete
    ...
    def applyTransform(self, transform: ThreeDimensionalTransform, auxiliarPoints = None, debug= False):
        ...
        if type(auxiliarPoints) == type(None):
            if debug: print("auxiliarPoints es None")
            return False
    ...
    def transformsThatCompletePoints(self,point: ThreeDimensionalPoint|list[ThreeDimensionalPoint], debug: False):
        if type(point) == list:
            ...
            transforms = ThreeDimensionalTransform.allPosibilitesNearbyOf(point, self.fM.pointsOrdered())
            auxTransforms = []
            if debug: print("                     transformsThatCompletePoints: estudiamos las transformaciones")
            for transform in transforms:
                if debug: print("                     transformación: {}".format(transform))
                auxiliarPoints = self.auxiliarPointsForTransform(transform)
                if type(auxiliarPoints) == type(None):
                    if debug: print("                     auxiliarPoints es None")
                    continue
                ...
                if point in pointsToComplete:
                    if debug: print("                     la añadimos")
                    auxTransforms.append(transform)
                else:
                    if debug: print("                     no completa el punto {}".format(point))
            ...
            if not point in pointsForAllInLineThatAreStudy:
                ...
                if axis != None:
                    ...
                    if len(allPoints)>2:
                        ...
                        if debug>1: print("Intentamos añadir en allInLine")
            else:
                if debug>1: print("Este ya está estudiado")
            ...
            for a in list(Axis):
                if not (point,a) in pointsForAllInAxisThatAreStudy:
                    ...
                    if len(allPoints)>3:
                        ...
                        if debug>1: print("Intentamos añadir en allInAxis")
            else:
                if debug>1: print("Este ya está estudiado")

        possibleTransforms = [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.slideCorner) for point in validPointsForThreeInCorner]
        possibleTransforms += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.allInLine,variety=False) for point in validPointsForAllInLineUndo]
        possibleTransforms += [ThreeDimensionalTransform(point,ThreeDimensionalTransformType.allInAxis,variety=None,direction=d)  for (point,axis) in validPointsForAllInAxis for d in [axis.axisDirection(1), axis.axisDirection(0)]]

        if debug>0: print("Repasados los puntos, hay {} posibilidades".format(len(possibleTransforms)))
        for transform in possibleTransforms:
            if debug>1: print("Intentando con transformación: {}".format(transform))
            auxiliarPoints = self.auxiliarPointsForTransform(transform)
            if type(auxiliarPoints) == type(None):
                if debug>1: print("Es none")
                continue
            _, _, pointsToDelete = auxiliarPoints
            if any([p in requirements  for p in pointsToDelete]):
                if debug>1: print("No cumple requirements")
                continue
            copy = deepcopy(self)
            copy.applyTransform(transform, auxiliarPoints)
            newLength = len(copy.fM.pointsOrdered())
            if newLength<actualLength:
                if debug>0: print("Mejorado",newLength)
    ...
    def reducesLength(self,requirements: list[ThreeDimensionalPoint] = [], debug = False):
        ...
        while True:
            ...
            if debug:
                print("Estamos en el ciclo {}, y tiene longitud {}".format(j,len(self.fM.pointsOrdered())))
                self.plot()
                #plt.pause(0.005)
            j+=1
        return transforms
