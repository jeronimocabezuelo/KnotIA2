...
def knotNames(n:int) -> Dict[int,List[str]]:
    auxDict: Dict[int,List[str]] = {numberCrosses: [] for numberCrosses in dict_knot_numbers.keys() if numberCrosses<= n}
...
def knotDatabase(maxCrosses: int, numberOfKnots: int, numberOfRandomMov: int = 100, maxStrands: int = None,debug:bool = False, initDB = None):
    ...
    for index, row in df.iterrows():
        ...
        #for i in range(numberOfKnots):
            ...
            if debug>0: print("name", name, len(knotSet))
            if debug>0: print("copy", i, ", percentage: {:3.2f}".format(((i)/numberOfKnots)*100))
            ...
            knot.randomMovN(numberOfRandomMov,maxStrands,percentage=True,debug=debug-1)
...
def createPDsForDatabase(db:pd.DataFrame):
    ...
    for index, row in db.iterrows():
        ...
        print("percentage: {:3.2f},time remaining:{}          ".format(percentage,remainingTimeString(remainingTime-(end-start))),end="\r")
    return pd.DataFrame(auxDict)
...
def createImagesForDatabase(path: str = "databases/masterPD.csv", limitNumberOfCrosses = 7, limitNumberOfImages = 100, debug = 0):
    ...
    for name in dictionary.keys():
        ...
        dictionary[name] = sorted(dictionary[name],key=lambda knot: max(knot.pd.shape[0],knot.pd.shape[1]))
        print("sorted")
    ...
    for name in dictionary.keys():
        ...
        if debug>0:
            ...
            print("   mean: {:3.2f} --> {:3.2f}".format(prevMean,sum(numberOfStrands)/len(numberOfStrands)))
    ...
    for name in dictionary.keys():
        ...
        for knot in dictionary[name]:
            i+=1
            print("knot {}, percentage {:3.2f}      ".format(name,100*i/len(dictionary[name])), end="\r")
            auxDict["name"].append(name)
    ...
    for i in range(len(auxDict["image"])):
        ...
        print("percentage normalize: {:3.2f}        ".format(100*(i+1)/len(auxDict["image"])),end="\r")
        pass
    print()
    for i in range(len(auxDict["image"])):
