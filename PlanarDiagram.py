...
def insertThreeRowOrColumn(matrix: np.ndarray, d: int, ind: Tuple[int, int]):
    unconnectedStrand = set(
        [matrix[auxInd] for auxInd in indicesNeedFree(ind, d) if matrix[auxInd] > 0])
    if d == 0:
...
def addCrossToMatrix(matrix: np.ndarray, cross: X, ind: Tuple[int, int]):
    ...
    isBelow = s % 2 == 0
    isUpDown = d % 2 == 0
    matrix[uDLF(d, uDLF(d, ind))] = (-1 if isUpDown else -
                                     2) if isBelow else (-2 if isUpDown else -1)
        ...
        for strand in self.unconnectedStrands:
            ...
            if len(indices) == 0:
                ...
                if debug:
                    ...
                    print(newPd)
                newUnconnected = [
                    s for s in self.unconnectedStrands if s != strand]
                for l in partDisconnected:
                    newUnconnected.append(l)
                    if l in newDict.keys():
                        newDict.pop(l)
                successorsArray.append(
                    NodePD(newPd, newUnconnected, crossCopy, newDict))
            else:
                gapsCreated = False
                length, matrixConnected, _, _ = connect(
                    self.pd, strand, strand)
                if type(matrixConnected) == type(None):
                    gapsCreated = True
                    length, matrixConnected, _, _ = connect(
                        createGaps(self.pd), strand, strand)
                if type(matrixConnected) == type(None):
                    if debug:
                ...
                if gapsCreated:
                    matrixConnected, newDict = compactPlanarDiagram(
                        matrixConnected, newDict)
                successorsArray .append(NodePD(matrixConnected, [
                                        s for s in self.unconnectedStrands if s != strand], deepcopy(self.remainCross), newDict))
...
def compactPlanarDiagram(pd: np.ndarray, lengths: Dict[Strand, int], debug=False):
    ...
    while True and c < 100:
        c += 1
        lengthsSorted = dict(
            sorted(lengths.items(), key=lambda item: item[1])).keys()
